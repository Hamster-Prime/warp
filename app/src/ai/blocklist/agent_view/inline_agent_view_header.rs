    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if is_agent_tagged_in {
            ...
            let message = if let Some(command) = top_level_command.as_deref() {
                format!("{} `{command}`", i18n::t!("Prompt agent to interact with"))
            } else {
                i18n::t!("Prompt agent to interact with the running command").to_string()
            };
        ...
        let message = if is_user_in_control {
            i18n::t!("User is in control").into_owned()
        } else if is_action_blocked {
            i18n::t!("Agent needs your permission to continue").into_owned()
        } else if is_waiting_for_command_to_exit {
            i18n::t!("Agent is waiting for command to exit").into_owned()
        } else if is_waiting_on_instructions {
            i18n::t!("Agent is waiting on instructions").into_owned()
        } else {
            i18n::t!("Agent is in control").into_owned()
        };
