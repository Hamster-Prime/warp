...
impl GlobalSearchView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let query_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(i18n::t!("Search in files").to_string(), ctx);
            editor
        });
        ...
        let case_sensitivity_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new_with_boxed_theme(String::new(), Arc::new(NakedTheme))
                ...
                .with_tooltip(i18n::t!("Toggle Case Sensitivity").to_string())
        ...
        let regex_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new_with_boxed_theme(String::new(), Arc::new(NakedTheme))
                ...
                .with_tooltip(i18n::t!("Toggle Regex").to_string())
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
...
impl View for GlobalSearchView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let search_label = Text::new_inline(i18n::t!("Search"), appearance.ui_font_family(), 14.)
        ...
        let message = if let Some(error) = &self.last_error {
            ...
            match self.total_match_count {
                1 => i18n::t!(
                    "1 result in {files} {file_word}",
                    files = files,
                    file_word = file_word
                )
                .to_string(),
                n => i18n::t!(
                    "{n} results in {files} {file_word}",
                    n = n,
                    files = files,
                    file_word = file_word
                )
        ...
        let capped_message = i18n::t!("The result set only contains a subset of all matches. Be more specific in your search to narrow down results.").to_string();
