    ...
    fn render_editor(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        Container::new(
            Flex::column()
                ...
                .with_child(
                    Container::new(
                        Container::new(Text::new(i18n::t!("JSON"), ui_font_family, font_size).finish())
        ...
        if contains_secrets {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(i18n::t!("This MCP server contains secrets. Visit Settings > Privacy to modify your secret redaction settings.").to_string()),
                    window_id,
                    ctx,
                );
            });
