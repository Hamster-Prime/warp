                        ...
                        Ordering::Equal | Ordering::Less => {
                            output_items.add_child(
                                render_informational_footer(
                                    app,
                                    i18n::t!("Sorry you had a bad experience with this interaction. We've refunded you 1 credit. We appreciate your feedback!").to_string(),
                                )
                                .with_agent_output_item_spacing(app)
                                .finish(),
                            );
                            ...
                            output_items.add_child(
                                render_informational_footer(
                                    ...
                                    i18n::t!(
                                        ...
                                        request_refunded_count = request_refunded_count
                                    )
                                    .to_string(),
                                )
                                .with_agent_output_item_spacing(app)
        ...
        Some(status) => match status {
            ...
            AIActionStatus::Blocked => {
                ...
                .with_header(blocked_action_header(
                    id.clone(),
                    &i18n::t!("Grant access to the following repository?"),
...
fn render_read_files(
    ...
    if status.as_ref().is_some_and(|status| status.is_blocked()) {
        ...
        renderable_action = renderable_action
            .with_header(blocked_action_header(
                id.clone(),
                &i18n::t!("Grant access to the following files?"),
                buttons.run_button.clone(),
                buttons.cancel_button.clone(),
                props.action_model,
                props.model,
...
fn create_formatted_text_for_grep(
    ...
    let formatted_text = if queries.len() == 1 {
        ...
        let mut fragments = if is_cancelled || is_queued {
            vec![
                FormattedTextFragment::plain_text(i18n::t!("Grep for ")),
                FormattedTextFragment::inline_code(query),
            ]
        } else {
            vec![
                FormattedTextFragment::plain_text(i18n::t!("Grepping for ")),
                FormattedTextFragment::inline_code(query),
            ]
        };
...
fn create_formatted_text_for_file_glob(
    ...
    let formatted_text = if patterns.len() == 1 {
        ...
        let mut fragments = if is_cancelled || is_queued {
            vec![
                FormattedTextFragment::plain_text(i18n::t!("Search for files that match ")),
                FormattedTextFragment::inline_code(pattern),
            ]
        } else {
            vec![
                FormattedTextFragment::plain_text(i18n::t!("Finding files that match ")),
                FormattedTextFragment::inline_code(pattern),
            ]
        };
        fragments.push(if is_cancelled {
...
fn render_file_retrieval_tool(
    ...
    if status.as_ref().is_some_and(|status| status.is_blocked()) {
        ...
        config = config
            .with_header(blocked_action_header(
                action_id.clone(),
                &i18n::t!("OK if I search the files in this directory?"),
                buttons.run_button.clone(),
                buttons.cancel_button.clone(),
                props.action_model,
                props.model,
...
fn render_upload_artifact(
    ...
    if status.as_ref().is_some_and(|status| status.is_blocked()) {
        ...
        renderable_action = renderable_action
            .with_header(blocked_action_header(
                action_id.clone(),
                &i18n::t!("Grant access to upload this artifact?"),
                buttons.run_button.clone(),
                buttons.cancel_button.clone(),
                props.action_model,
                props.model,
