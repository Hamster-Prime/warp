...
impl TabConfigParamsModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let submit_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(i18n::t!("Open Tab"), PrimaryTheme)
                .with_keybinding(
        ...
        let submit_button_disabled =
            ctx.add_typed_action_view(|_| ActionButton::new(i18n::t!("Open Tab"), DisabledTheme));
        Self {
            ...
            let field = match param.param_type {
                ...
                TabConfigParamType::Text => {
                    ...
                    let placeholder = if default_text.is_empty() {
                        i18n::t!("Enter {name}", name = name).to_string()
                    } else {
                        default_text.clone()
                    };
