    ...
    pub fn new(
        ...
        let mic_button = ctx.add_typed_action_view(|_ctx| {
            let button = ActionButton::new("", ActiveMicButtonTheme)
                ...
                .with_tooltip(i18n::t!("Voice input").to_string())
                .with_size(button_size)
                .with_tooltip_alignment(TooltipAlignment::Left);
            #[cfg(feature = "voice_input")]
        ...
        let file_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("", AgentInputButtonTheme)
                .with_icon(Icon::Plus)
                .with_tooltip(i18n::t!("Attach file").to_string())
                .with_size(button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
                .on_click(|ctx| {
        ...
        let handoff_to_cloud_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("", AgentInputButtonTheme)
                ...
                .with_tooltip(i18n::t!("Hand off to cloud (or type &)").to_string())
                .with_size(button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
                .on_click(|ctx| {
        ...
        let settings_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("", AgentInputButtonTheme)
                ...
                .with_tooltip(i18n::t!("Open coding agent settings").to_string())
                .with_size(cli_button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
                .on_click(|ctx| {
        ...
        let dismiss_plugin_chip_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("", InstallPluginButtonTheme)
                .with_icon(Icon::X)
                .with_size(cli_button_size)
                .with_tooltip(i18n::t!("Dismiss").to_string())
                .with_tooltip_alignment(TooltipAlignment::Left)
        ...
        ctx.subscribe_to_model(
            ...
            move |me, _, event, ctx| {
                ...
                me.rich_input_button.update(ctx, |button, ctx| {
                    if is_open {
                        button.set_label(i18n::t!("Hide Rich Input"), ctx);
                        button.set_tooltip(Some(i18n::t!("Hide Rich Input")), ctx);
                        ...
                        button.set_label(i18n::t!("Rich Input"), ctx);
                        button.set_tooltip(Some(i18n::t!("Open Rich Input")), ctx);
                        button.set_keybinding(
                            Some(KeystrokeSource::Binding(
                                OPEN_CLI_AGENT_RICH_INPUT_KEYBINDING,
                            )),
        ...
        let context_window_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("", AgentInputButtonTheme)
                ...
                .with_tooltip(i18n::t!("Context window usage").to_string())
                .with_size(button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
        });
        ...
        match &self.cli_voice_input_state {
            CLIVoiceInputState::Stopped => {
                if !crate::ai::AIRequestUsageModel::as_ref(ctx).can_request_voice() {
                    self.show_cli_voice_error_toast(&i18n::t!("Voice input limit reached"), ctx);
                    return;
                }
            ...
            Err(e) => match e {
                ...
                _ => {
                    ...
                    self.show_cli_voice_error_toast(
                        &i18n::t!("Failed to transcribe voice input"),
                        ctx,
                    );
                }
            },
    ...
    fn show_cli_microphone_access_toast(&self, ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::error(
                i18n::t!("Failed to start voice input (you may need to enable Microphone access)")
                    .to_string(),
            );
            toast_stack.add_ephemeral_toast(toast, window_id, ctx);
        });
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::success(
                        i18n::t!(
                            ...
                            key = toggle_key.display_name().to_string()
                        )
                        .to_string(),
                    );
                    toast_stack.add_ephemeral_toast(toast, window_id, ctx);
