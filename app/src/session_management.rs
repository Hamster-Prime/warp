    ...
    pub fn a11y_description(&self) -> Option<String> {
        match self {
            ...
            Self::LastRunCommand {
                ...
                i18n::t!("Last run command {clone}", clone = last_run_command.clone()).to_string(),
            ),
            Self::LastRunAIBlock { prompt } => {
                Some(i18n::t!("Last AI interaction: {prompt}", prompt = prompt).to_string())
            }
            Self::RunningCommand { running_command } => Some(
                i18n::t!(
                    "Currently running {running_command}",
            ...
            Self::RunningAIBlock { prompt } => Some(
                i18n::t!(
                    "Currently running AI interaction: {prompt}",
                    prompt = prompt
                )
                .to_string(),
            ),
